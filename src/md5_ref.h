// md5_ref.h — minimal, correct MD5 (RFC 1321). CPU reference (compile with g++).
// Little-endian digest output, matching OpenSSL / hashcat inc_hash_md5.
#pragma once
#include <cstdint>
#include <cstring>
#include <cstddef>
...
struct MD5 {
    ...
    void update(const void* data, size_t len) {
        ...
        total += len;
        while (len) {
            size_t n = 64 - buflen; if (n > len) n = len;
...
// one-shot helper
inline void md5(const void* data, size_t len, uint8_t out[16]) {
    MD5 m; m.update(data, len); m.final(out);
}
